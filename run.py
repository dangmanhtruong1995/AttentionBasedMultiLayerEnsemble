...
def main():
    ...
    # config['optimizer_early_stopping'] = 5
    ...
    for file_name in file_list:
        ...
        the_dataset_path = os.path.join(os.getcwd(), 'result', file_name)
        if not os.path.exists(the_dataset_path):
            os.makedirs(the_dataset_path)

        # Run the algorithm
        deep_ensemble = DeepEnsemble(config, file_name)
        t1 = time()
        ...
        # Encoding file
        print("Output encoding file!")
        encoding_list = encoding_list[:n_layers+1]
        with open(os.path.join(output_path, 'encoding.txt'), 'wt') \
            ...
            for i_layer, encoding in enumerate(encoding_list):
                print("Encoding for layer %d" % (i_layer))
                print(encoding)
                if i_layer > n_layers:
                    continue
                fid.write('Layer %d:\n' % (i_layer + 1))
                if i_layer == 0:
                    ...
                    for i_clfs in range(n_clfs):
                        ...
                        print("-- Encoding for classifier %d: " % (i_clfs))
                        print(enc_clfs_i)
